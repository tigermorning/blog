...
window.BIGMAP = {
 ...
 "world": {
  ...
  "lead": "CS 상담 에이전트 글의 예시처럼, 쇼핑몰 챗봇에 누군가 \"반품 배송비 얼마예요?\"라고 적었다고 해 볼게요. 상자마다 \"이 질문에서\" 칸에 그 단계가 답을 만드는 데 어떤 몫을 맡는지 적었어요. 번호를 따라가 보고, 궁금한 상자를 눌러 들어가세요.",
  "nodes": [
   {
    "id": "user",
    "label": "사용자",
    "sub": "\"반품 배송비 얼마예요?\"를 입력"
   },
   {
    "id": "bench",
    "label": "개발 환경·Git",
    "sub": "서비스를 만들고 고치고 내보내는 곳",
    "chips": [
     "터미널",
     "Claude Code",
     "Git·GitHub",
    ...
    "share": "이 챗봇의 코드를 코딩 에이전트와 짜고, 커밋해 배포해 둔 상태예요. 질문이 오기 전에 할 일이 끝나 있어요."
   },
   {
    "id": "web",
    "label": "웹 서비스",
    "step": "1",
    "sub": "화면이 질문을 서버로 보내고, 서버는 API 키를 쥐고 대화를 DB에 남기는 곳",
    "chips": [
     "프론트엔드",
     "REST API",
     "백엔드",
    ...
    "share": "질문을 서버까지 전달해요. 아직 답에 필요한 건 아무것도 몰라요."
   },
   {
    "id": "agent",
    "label": "오케스트레이션(에이전트 흐름 설계)",
    "step": "2",
    "sub": "모델 호출의 순서·분기·반복을 정하는 코드",
    "chips": [
     "LangGraph",
     "State·Node·Edge",
     "라우팅",
    ...
    "share": "\"배송비\"라는 낱말이 있어도, 고객이 원하는 건 반품이므로 RETURN_REFUND로 분류해요. 이 판정이 ③에서 어느 매뉴얼 장을 넣을지 정해요."
   },
   {
    "id": "senses",
    "label": "멀티모달",
    "sub": "사진·문서·소리를 읽고, 그림·3D를 만드는 모델",
    "chips": [
     "OCR",
     "STT·Whisper",
     "VLM",
    ...
    "share": "사진·음성이 없는 글자 질문이라 이번엔 건너뛰어요."
   },
   {
    "id": "talk",
    "label": "프롬프트·컨텍스트 엔지니어링",
    "step": "3",
    "sub": "모델을 부를 때마다 입력을 짜는 설계",
    "chips": [
     "지시 설계",
     "컨텍스트 창",
     "RAG",
    ...
    "share": "매뉴얼 중 교환·반품·환불 장을 넣어요. 그 장에 \"반품 배송비 5,000원\"이 고정값으로 적혀 있어서, 답의 근거가 여기서 들어와요."
   },
   {
    "id": "brain",
    "label": "머신러닝·LLM",
    "step": "4",
    "sub": "입력을 읽고 다음 토큰을 계산해 답을 쓰는 모델",
    "chips": [
     "트랜스포머",
     "토큰",
     "로컬 LLM·API"
    ],
    "share": "넣어 준 장에서 5,000원을 찾아 답 초안을 써요. 매뉴얼 고정값이라 도구를 부를 필요가 없어요."
   },
   {
    "id": "tools",
    "label": "RAG·MCP·자동화",
    "step": "5",
    "sub": "모델 대신 실제 값을 가져오는 도구·검색·자동화",
    "chips": [
     "도구 호출",
     "MCP 서버",
     "벡터 검색"
    ],
    "share": "이번엔 쉬어요. \"제 반품 건 배송비는 누가 내요?\"처럼 건마다 다른 값이면 get_return_status로 검품 단계를 조회해요."
   },
   {
    "id": "check",
    "region": "agent",
    "link": "#/agent/cs-agent-project",
    "label": "답 검사 → 화면으로",
    "step": "6",
    "sub": "모델이 쓴 답을 내보내기 전에 확인하는 코드",
    "chips": [
     "가드레일",
     "그라운딩",
     "평가"
    ],
    "share": "답 속 5,000이 매뉴얼 고정값 목록에 있으니 통과해요. \"반품 배송비는 5,000원이에요\"가 화면에 떠요."
   }
  ],
  "edges": [
   [
  ...
  "steps": [
   ...
   [
    "② 오케스트레이션",
    "질문이 배송(SHIPPING)·교환·반품·환불(RETURN_REFUND) 같은 라우트 중 어디로 보낼지 분류하고(라우팅) 처리 순서를 정해요. 사진·음성이 붙어 있으면 멀티모달 단계를 먼저 거치게 해요. 여기서 오케스트레이션은 오케스트레이터-워커 패턴 하나가 아니라, 호출 순서·분기·반복을 정하는 일 전체예요."
   ],
   [
    "(있을 때만) 멀티모달",
    "사진·음성을 글자로 바꾸거나(OCR·STT), 이미지를 그대로 둔 채(VLM·옴니) ③으로 넘겨요."
   ],
   [
    "③ 프롬프트·컨텍스트 엔지니어링",
    "모델을 부를 때마다 입력을 짜는 설계예요. 오케스트레이션 코드가 호출 직전에 지시문, 매뉴얼에서 찾아 온 관련 조각(RAG), 쓸 수 있는 도구 목록, 사진에서 뽑은 글자나 이미지를 입력 하나로 조립해요."
   ],
   [
    "④ 머신러닝·LLM",
    "모델이 입력을 읽고 답 초안을 써요. 입력에 없는 건별 값이 필요할 때만 도구 호출을 요청하고, 조회 결과를 받아 다시 써요."
   ],
   [
    "⑤ RAG·MCP·자동화",
   ...
   [
    "⑥ 답 검사",
    "모델이 쓴 답 초안은 바로 나가지 않아요. 오케스트레이션 코드의 가드레일이 숫자의 출처를 확인한 뒤, ①을 거꾸로 타고 화면으로 보내요."
   ],
   [
    "만드는 곳",
    "이 전부는 개발 환경에서 코딩 에이전트와 Git으로 만들어 배포해요."
   ]
  ]
 },
 "threads": [
