...
window.BIGMAP = {
 ...
 "world": {
  ...
  "steps": [
   ...
   [
    "② 오케스트레이션",
    "질문이 환불·배송 같은 라우트 중 어디로 보낼지 분류하고(라우팅) 처리 순서를 정해요. 사진·음성이 붙어 있으면 멀티모달 단계를 먼저 거치게 해요. 여기서 오케스트레이션은 오케스트레이터-워커 패턴 하나가 아니라, 호출 순서·분기·반복을 정하는 일 전체예요."
   ],
   [
    "(있을 때만) 멀티모달",
